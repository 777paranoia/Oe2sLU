...
def _convert_to_mono(e2s_sample, mix):
    ...
    if num_chans > 1:
        ...
        # wav_mchan_to_mono expects a per-channel list of samples, not raw
        # bytes, and returns the same shape -- convert in and back out.
        _wav = wav_tools.wav_from_raw16b(
            e2s_sample.get_data().rawdata, prev_fmt.channels)
        _mono = wav_tools.wav_mchan_to_mono(_wav, w)
        _data = wav_tools.raw16b_from_wav(_mono)
