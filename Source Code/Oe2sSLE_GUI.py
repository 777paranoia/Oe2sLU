...
class SliceEditor(tk.PanedWindow):
    def __init__(self, *args, **kwargs):
        ...
        # Bottom area: left = sample options (scrollable), right = active steps
        # (scrollable), with a draggable divider.
        self.bottom = tk.PanedWindow(self, orient='horizontal',
                                     sashrelief='raised', sashwidth=6)
        self.add(self.bottom)
        self.frame = VerticalScrolledFrame(self.bottom)
        self.bottom.add(self.frame, minsize=240, sticky='nsew', stretch='always')
        self.rightFrame = VerticalScrolledFrame(self.bottom)
        self.bottom.add(self.rightFrame, minsize=240, sticky='nsew',
                        stretch='always')
        ...
        frame = tk.Frame(self.rightFrame.interior)
        frame.pack()
        ...
        self.activeStepsEntry = [None]*64
        COLS = 8
        for g in range(64 // COLS):
            tk.Label(frame, text="Step : ").grid(row=g*2+0, column=0)
            tk.Label(frame, text="Slice : ").grid(row=g*2+1, column=0)
            for i in range(COLS):
                idx = g*COLS + i
                tk.Label(frame, text=idx+1).grid(row=g*2+0, column=i+1)
                sb = ROSpinbox(frame, values=("Off",)+tuple(range(64)), width=3, textvariable=self.activeSteps[idx])
                sb.grid(row=g*2+1, column=i+1)
                self.activeStepsEntry[idx] = sb

        tk.Button(frame, text="Off them all", command=self.allActiveStepsOff).grid(row=16, column=0, columnspan=COLS+1, padx=5, pady=5)
...
class SliceEditorDialog(tk.Toplevel):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        if width > 0 and height > 0:
            ...
            # On macOS geometry may not be ready yet; defer until after first draw
            self.after(100, lambda: self.minsize(
                self.sliceEditor.winfo_reqwidth() or width,
                self.sliceEditor.winfo_reqheight() or height))
...
class Sample(object):
    ...
    def __init__(self, master, line_num, sample_num):
        ...
        self.entryName = tk.Entry(self.frame, width=20, textvariable=self.name)
...
class SampleList(tk.Frame):
    ...
    def edit(self, smpl_num):
        # Expand the slice editor inline at the bottom of the main window
        # rather than opening a separate Toplevel.
        self.parent.show_inline_slice_editor(self, smpl_num)
...
class SampleAllEditor(tk.Frame):
    """
    Embeddable: this used to subclass tk.Tk. It now lives in a Frame so it can
    be dropped into another window (e.g. the slicer's collapsible panel) while
    still running standalone via the __main__ block below.
    ...
    # window-only calls redirected to the containing toplevel
    def wm_title(self, *a, **k):
        return self.winfo_toplevel().wm_title(*a, **k)
    title = wm_title

    def minsize(self, *a, **k):
        return self.winfo_toplevel().minsize(*a, **k)

    def show_inline_slice_editor(self, smpl_list, smpl_num):
        """Open the visual slice editor as a panel at the bottom of this
        window instead of a separate Toplevel."""
        top = self.winfo_toplevel()
        if hasattr(top, "host_show_slice_editor"):
            # combined window owns the bottom full-width slice panel
            top.host_show_slice_editor(smpl_list, smpl_num)
            return
        if getattr(self, "_inlineSliceWrap", None) is None:
            self._inlineSliceWrap = tk.Frame(self, borderwidth=2,
                                             relief="sunken")
            bar = tk.Frame(self._inlineSliceWrap)
            tk.Label(bar, text="Slice editor").pack(side=tk.LEFT, padx=4)
            tk.Button(bar, text="Close slice editor",
                      command=self.hide_inline_slice_editor).pack(
                side=tk.RIGHT, padx=4, pady=2)
            bar.pack(fill=tk.X)
            self._inlineSlice = SliceEditor(self._inlineSliceWrap)
            self._inlineSlice.pack(fill=tk.BOTH, expand=tk.YES)
        self._inlineSlice.set_sample(smpl_list, smpl_num)
        self._inlineSliceWrap.pack(side=tk.BOTTOM, fill=tk.BOTH, expand=tk.YES)

    def hide_inline_slice_editor(self):
        try:
            audio.player.play_stop()
        except Exception:
            pass
        if getattr(self, "_inlineSliceWrap", None) is not None:
            self._inlineSliceWrap.pack_forget()
        ...
        if nextsampleIndex is not None:
            ...
            if len(self.sampleList.samples) == 1:
                ...
                if width > 0 and height > 0:
                    ...
                    self.after(100, lambda: self.minsize(
                        self.winfo_reqwidth() or width,
                        self.winfo_reqheight() or height))
        else:
            raise ToManySamples
